/*
 *
 *  Microstrain 3DM-GX3 communication
 *  Adapted by Patrick Beeson (pbeeson@traclabs.com)
 *  from the Player driver
...
#define IMU_EXCEPT(except, msg, ...) \
  { \
    char buf[1000]; \
    snprintf(buf, 1000, msg" (in microstrain_3dm_gx3::IMU:%s)", ##__VA_ARGS__, __FUNCTION__); \
    throw except(buf); \
  }

// Some systems (e.g., OS X) require explicit externing of static class
// members.
extern const double microstrain_3dm_gx3::IMU::G;
extern const double microstrain_3dm_gx3::IMU::KF_K_1;
extern const double microstrain_3dm_gx3::IMU::KF_K_2;
...
////////////////////////////////////////////////////////////////////////////////
// Constructor
microstrain_3dm_gx3::IMU::IMU() : fd(-1), continuous(false)
{ }
...
// Open the IMU port
void
microstrain_3dm_gx3::IMU::openPort(const char *port_name)
{
  ...
  if (fd < 0)
  {
    ...
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Unable to open serial port [%s]. %s. %s", port_name, strerror(errno), extra_msg);
  }
  ...
  if (fcntl(fd, F_SETLK, &fl) != 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Device %s is already locked. Try 'lsof | grep %s' to find other processes that currently have the port open.", port_name, port_name);

  // Change port settings
  struct termios term;
  if (tcgetattr(fd, &term) < 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Unable to get serial port attributes. The port you specified (%s) may not be a serial port.", port_name);
  ...
  if (tcsetattr(fd, TCSAFLUSH, &term) < 0 )
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Unable to set serial port attributes. The port you specified (%s) may not be a serial port.", port_name); /// @todo tcsetattr returns true if at least one attribute was set. Hence, we might not have set everything on success.
  ...
  if (tcflush(fd, TCIOFLUSH) != 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Tcflush failed. Please report this error if you see it.");
}
...
// Close the IMU port
void
microstrain_3dm_gx3::IMU::closePort()
{
  if (fd != -1)
  {
    if (continuous)
    ...
    if (close(fd) != 0)
      IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Unable to close serial port; [%s]", strerror(errno));
    fd = -1;
  }
}
...
void
microstrain_3dm_gx3::IMU::initTime(double fix_off)
{
  wraps = 0;
  ...
  uint64_t start_time_first = time_helper();
  transact(cmd, sizeof(cmd), rep, sizeof(rep), 1000);
  start_time = time_helper();
  start_time = start_time/2+start_time_first/2;
...
void
microstrain_3dm_gx3::IMU::initGyros(double* bias_x, double* bias_y, double* bias_z)
{
  wraps = 0;
...
bool
microstrain_3dm_gx3::IMU::setContinuous(cmd command)
{
  uint8_t cmd[4];
  uint8_t rep[8];
...
void
microstrain_3dm_gx3::IMU::stopContinuous()
{
  uint8_t cmd[3];
  ...
  if (tcflush(fd, TCIOFLUSH) != 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "Tcflush failed");
...
void
microstrain_3dm_gx3::IMU::receiveAccelAngrateMag(uint64_t *time, double accel[3], double angrate[3], double mag[3])
{
  int i, k;
  uint8_t rep[43];
...
void
microstrain_3dm_gx3::IMU::receiveAccelAngrateOrientation(uint64_t *time, double accel[3], double angrate[3], double orientation[9])
{
  int i, k;
  uint8_t rep[67];
...
void
microstrain_3dm_gx3::IMU::receiveAccelAngrate(uint64_t *time, double accel[3], double angrate[3])
{
  int i, k;
  uint8_t rep[31];
...
void
microstrain_3dm_gx3::IMU::receiveEuler(uint64_t *time, double *roll, double *pitch, double *yaw)
{
  uint8_t rep[19];
...
bool microstrain_3dm_gx3::IMU::getDeviceIdentifierString(id_string type, char id[17])
{
  uint8_t cmd[2];
  uint8_t rep[20];
...
void 
microstrain_3dm_gx3::IMU::receiveAccelAngrateMagOrientation (uint64_t *time, double accel[3], double angrate[3], double mag[3], double orientation[9]) 
{
...
void
microstrain_3dm_gx3::IMU::receiveRawAccelAngrate(uint64_t *time, double accel[3], double angrate[3])
{
  int i, k;
  ...
  receive(microstrain_3dm_gx3::IMU::CMD_RAW, rep, sizeof(rep), 1000, &sys_time);
...
uint64_t
microstrain_3dm_gx3::IMU::extractTime(uint8_t* addr)
{
...
int microstrain_3dm_gx3::IMU::transact(void *cmd, int cmd_len, void *rep, int rep_len, int timeout)
{
  send(cmd, cmd_len);
...
int
microstrain_3dm_gx3::IMU::send(void *cmd, int cmd_len)
{
  int bytes;
  ...
  if (bytes < 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "error writing to IMU [%s]", strerror(errno));

  if (bytes != cmd_len)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "whole message not written to IMU");

  // Make sure the queue is drained
  // Synchronous IO doesnt always work
  if (tcdrain(fd) != 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "tcdrain failed");
...
static int read_with_timeout(int fd, void *buff, size_t count, int timeout)
{
  ...
  if ( (retval = poll(ufd, 1, timeout)) < 0 )
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "poll failed  [%s]", strerror(errno));

  if (retval == 0)
    IMU_EXCEPT(microstrain_3dm_gx3::TimeoutException, "timeout reached");
	
  nbytes = read(fd, (uint8_t *) buff, count);

  if (nbytes < 0)
    IMU_EXCEPT(microstrain_3dm_gx3::Exception, "read failed  [%s]", strerror(errno));
...
// Returns the number of bytes read.
int
microstrain_3dm_gx3::IMU::receive(uint8_t command, void *rep, int rep_len, int timeout, uint64_t* sys_time)
{
  ...
  *(uint8_t*)(rep) = 0;
  ...
  if (checksum != bswap_16(*(uint16_t*)((uint8_t*)rep+rep_len-2)))
    IMU_EXCEPT(microstrain_3dm_gx3::CorruptedDataException, "invalid checksum.\n Make sure the IMU sensor is connected to this computer.");
...
uint64_t microstrain_3dm_gx3::IMU::filterTime(uint64_t imu_time, uint64_t sys_time)
{
...
double microstrain_3dm_gx3::IMU::toDouble(uint64_t time)
{
...
uint64_t  microstrain_3dm_gx3::IMU::toUint64_t(double time)
{
